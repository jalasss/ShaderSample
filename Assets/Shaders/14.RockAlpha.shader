...
Shader "YWY/14.Rock Alpha"
{
	...
	SubShader{
			Tags{"Queue"="Transparent" "IngnoreProjector"="true" "RenderType"="Transparent" }	//透明标签，前面是渲染队列，透明最后渲染 ，后面的先不管什么意思
		Pass{
			Tags{"LightMode" = "ForwardBase"}

			Zwrite Off							//深度写入
			Blend SrcAlpha OneMinusSrcAlpha		//加上这两句之后，rgba的a就生效了，可以在片元函数返回值中设置alpha
			...
			//	fixed3 tangentNormal = normalize(normalColor.xyz*2 -1); //算出真正法线方向在切线空间下的值(一般不使用这种方法)
				...
				return fixed4(tempColor,_AlphaScale);	//color值范围0~1,因为设置了深度写入，所以可以设置alpha的值了
			}
