...
Shader "YWY/09.Specular Vertex BlinnPhong"
{
	...
	SubShader{
		Pass{
			...
			v2f vert(a2v v) {  
				...
				fixed3 blinnPhongDir = normalize(lightDir + viewDir);	//入射光方向和视线方向夹角的一半

				fixed3 specular = _LightColor0.rgb * _Specular.rgb*pow(max(dot(normalDir,blinnPhongDir),0),_Gross); //BlinnPhong
