	...
	SubShader{
		Pass{
			...
			fixed4 frag(v2f f) :SV_Target{
				...
				fixed3 viewDir = normalize(_WorldSpaceCameraPos.xyz - f.worldVetex);

				fixed3 halfDir = normalize(viewDir + lightDir);

				fixed3 specular = _LightColor0.rgb * _Specular.rgb*pow(max(dot(normalDir,halfDir),0),_Gross); //blinnphong

				fixed3 tempColor = diffuse+ambient*tempTexColor+ specular;  //ambient*tempTexColor是为了纹理颜色与环境光相融合
